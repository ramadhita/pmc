...
int main(void){
    ...
    max_nama = min_n(database);
    min_nilai = min(database);
    min_nama = min_n(database);
    rerata = avg(database);

    // output
    printf("Nilai maks %d oleh %s", max_nilai, database.datapoint[max_nama].nama);
    printf("\nNilai min %d oleh %s",min_nilai, database.datapoint[min_nama].nama);
